...
python train.py "--worker_num" ${MS_WORKER_NUM} --enable_distribute True > scheduler.txt 2>&1 &
sched_pid=${!}
...
do
    python train.py "--worker_num" ${MS_WORKER_NUM} --enable_distribute True > worker_$i.txt 2>&1 &
