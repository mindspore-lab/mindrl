...
# ============================================================================
"""RNN operators module, include RNN, GRU and LSTM."""
...
class _DynamicGRUCPUGPU(Cell):
    ...
    def construct(self, x, h_0, seq_length, w_ih, w_hh, b_ih, b_hh):
        ...
        if self.is_gpu and self.enable_fusion:
            if b_ih is None:
                weights = self.concat((
                    w_ih.view(-1, 1, 1),
                    w_hh.view(-1, 1, 1)
...
class _RNNBase(Cell):
    ...
    def __init__(self, mode, input_size, hidden_size, num_layers=1, has_bias=True,
                 batch_first=False, dropout=0., bidirectional=False, enable_fusion=True):
        super().__init__()
        ...
        if mode == "LSTM":
            ...
            self.rnn = _DynamicGRUAscend() if is_ascend else _DynamicGRUCPUGPU(enable_fusion=enable_fusion)
        elif mode == "RNN_TANH":
...
class RNN(_RNNBase):
    ...
    def __init__(self, *args, **kwargs):
        if 'nonlinearity' in kwargs:
            if kwargs.get('nonlinearity') == 'tanh':
                mode = 'RNN_TANH'
            elif kwargs.get('nonlinearity') == 'relu':
                mode = 'RNN_RELU'
            else:
                raise ValueError(f"For '{self.cls_name}', the 'nonlinearity' must be in ['tanh', 'relu'], "
                                 f"but got {kwargs.get('nonlinearity')}.")
