...
class GenerateFragment():
    ...
    def get_trainer_by_algo_name(cls, algo_name) -> str:
        ...
        trainer_file = cur + "/../../algorithm/" + algo_name + "/" + algo_name + "_trainer.py"
        return trainer_file
    ...
    @classmethod
    def find_buffer_and_learn_args(cls, ast_source,
                                    buffer_func=[
                                        'get_replay_buffer_elements', 'replay_buffer_sample'],
        ...
        for nodes in ast.iter_child_nodes(ast_source):
            for node in ast.iter_child_nodes(nodes):
                if isinstance(node, ast.FunctionDef) and node.name == 'train_one_episode':
                    for i in node.body:
                        ...
                        if isinstance(i, ast.AugAssign) and isinstance(i.value, ast.Call):
                            if i.value.func.attr in learn_func:
    ...
    def replace_augassign(self, ast_body, learn_func=['agent_learn']):
        ...
        if not isinstance(ast_body, list):
            ...
            node = self.find_func(ast_body, learn_func)
        if node:
            new_node = ast.Assign(lineno=node[1].lineno, col_offset=node[1].col_offset,
                                  targets=[node[1].target], value=node[1].value)
            ...
            for node in ast.iter_child_nodes(nodes):
                if isinstance(node, ast.FunctionDef) and node.name == 'train_one_episode':
                    for n_body in node.body:
                        if isinstance(n_body, (ast.AugAssign, ast.Assign)) \
                            and isinstance(n_body.value, ast.Call) and n_body.value.func.attr in func:
                            print("Find boundary function ",
                                  n_body.value.func.attr)
                            boundary = True
                        if boundary:
    ...
    @classmethod
    def build_send(self, frag_type):
        '''TODO'''
        return None, None

    @classmethod
    def build_allgather(cls, frag_type, user_defined_buffer_name, framework_buffer_name, framework_weight_name, learn_args, learn_return):
        '''
        ...
        For the usage of `Actor`, we AllGather the weight of each rank, and get the first dim of it(the rank 0 replicate learner).
        the assign the weight into local policy net. These code will add in the top of the Actor part, we call it top_op_nodes.
        then we will expand first dims for the buffer created by the each actor and choose the dims except the first(the rank 0),
        and AllGather the buffer at last, we call these code bottom_op_nodes.
        ...
        if frag_type == 'Actor':
            ...
            for i, b in enumerate(buffer_name):
                if len(user_defined_buffer_name) == 1:
                    arg = ast.Subscript(value=ast.Name(buffer_name[0], ctx=ast.Load()),
                ...
                expand_node = ast.Assign(targets=[ast.Name(id=tmp_src_buffer, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.expanddims', ctx=ast.Load()),
                                         args=[ast.Name(id=arg), ast.Num(0)], keywords=[]))
                assign_node = ast.Assign(targets=[ast.Name(id=src_buffer, ctx=ast.Store())],
                                         value=ast.Name(id=tmp_src_buffer), ctx=ast.Load())
                allgather_node = ast.Assign(targets=[ast.Name(id=tmp_buffer_all, ctx=ast.Store())],
                                            value=ast.Call(func=ast.Name(id='self.allgather', ctx=ast.Load()),
                                            args=[ast.Name(id=src_buffer)], keywords=[]))
                depend_node = ast.Assign(targets=[ast.Name(id=tmp_buffer_all, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp_buffer_all),
                                                              ast.Name(id='all_buffer_' + str(i-1))], keywords=[]))
                ...
                if i == 0:
                    depend_node = ast.Assign(targets=[ast.Name(id=tmp_buffer_all, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp_buffer_all),
                                                              ast.Name(id=tmp_src_buffer)], keywords=[]))
            ...
            for i in range(len(framework_weight_name)):
                ...
                allgather_node = ast.Assign(targets=[ast.Name(id=tmp_data, ctx=ast.Store())],
                                            value=ast.Call(func=ast.Name(id='self.allgather',
                                                                         ctx=ast.Load()), args=[ast.Name(id=src_weight)], keywords=[]))
                assign_node = ast.Assign(targets=[ast.Name(id=dst_weight.strip("self."), ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.assign', ctx=ast.Load()),
                                                        args=[ast.Name(id=dst_weight), ast.Name(id=tmp_data+'[0,:]')], keywords=[]))
                depend_node = ast.Assign(targets=[ast.Name(id=tmp_data, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp_data),
                                                              ast.Name(id=net_weight_head.strip("self.")+ '_' + str(i-1))], keywords=[]))
                if i == 0:
                    depend_node = ast.Assign(targets=[ast.Name(id=tmp_data, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp_data),
                                                              ast.Name(id='all_buffer_' + str(len(buffer_name)-1))], keywords=[]))
                bottom_op_nodes.append(allgather_node)
                bottom_op_nodes.append(depend_node)
                bottom_op_nodes.append(assign_node)
        ...
        if frag_type == 'Learner':
            ...
            for i, b in enumerate(buffer_name):
                ...
                allgather_node = ast.Assign(targets=[ast.Name(id=tmp_buffer_all, ctx=ast.Store())],
                                            value=ast.Call(func=ast.Name(id='self.allgather', ctx=ast.Load()),
                                            args=[ast.Name(id=src_buffer)], keywords=[]))
                assign_node = ast.Assign(targets=[ast.Name(id=learn_arg, ctx=ast.Store())],
                                         value=ast.Subscript(value=ast.Name(id=tmp_buffer_all, ctx=ast.Load()),
                                         slice=ast.Slice(lower=ast.Num(1), upper=None, step=None), ctx=ast.Load()))
                depend_node = ast.Assign(targets=[ast.Name(id=tmp_buffer_all, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp_buffer_all),
                                                              ast.Name(id='all_buffer_' + str(i-1))], keywords=[]))
                reshape_node = ast.Assign(targets=[ast.Name(id=learn_arg, ctx=ast.Store())],
                                          value=ast.Call(func=ast.Name(id=learn_arg+'.reshape', ctx=ast.Load()),
                                                        args=[ast.Num(n=-1),
                                                              ast.Name(id=learn_arg+'.shape[-2]'),
                                                              ast.Name(id=learn_arg+'.shape[-1]')], keywords=[]))
            ...
            if len(learn_args) == 1:
                ...
                top_op_nodes.append(assign_node)
            # generate bottom node
            depend_node = ast.Assign(targets=[ast.Name(id=learn_return[0], ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=learn_return[0]),
                                                              ast.Name(id="all_buffer_"+str(len(buffer_name)-1))],
                                                        keywords=[]))
            bottom_op_nodes.append(depend_node)
            print_node = ast.Expr(value=ast.Call(func=ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()),
                                                                    attr="print", ctx=ast.Load()),
                                                 args=[ast.Str(s=learn_return[0]),
                                                       ast.Name(id=learn_return[0], ctx=ast.Load())], keywords=[]))
            ...
            for i in range(len(framework_weight_name)):
                ...
                assign_node = ast.Assign(targets=[ast.Name(id=dst_weight, ctx=ast.Store())],
                                         value=ast.Name(id=src_weight), ctx=ast.Load())
                allgather_node = ast.Assign(targets=[ast.Name(id=tmp, ctx=ast.Store())],
                                            value=ast.Call(func=ast.Name(id='self.allgather', ctx=ast.Load()),
                                            args=[ast.Name(id=dst_weight)], keywords=[]))
                depend_node = ast.Assign(targets=[ast.Name(id=tmp, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp),
                ...
                if i > 0:
                    ...
                    depend_node = ast.Assign(targets=[ast.Name(id=tmp, ctx=ast.Store())],
                                         value=ast.Call(func=ast.Name(id='self.depend', ctx=ast.Load()),
                                                        args=[ast.Name(id=tmp),
                                                              ast.Name(id=dst_weight)], keywords=[]))
                    bottom_op_nodes.append(depend_node)


        return top_op_nodes, bottom_op_nodes

    def gen_commuicate_statement(self, frag_type, communication_op, user_defined_buffer_name,
                                  framework_buffer_name, framework_weight_name, learn_args, learn_return):
        ...
        if 'AllGather' in communication_op:
            top_state, bottom_state = self.build_allgather(
                frag_type, user_defined_buffer_name, framework_buffer_name, framework_weight_name, learn_args, learn_return)
    ...
    def insert_communication_states(self, ast_target, ast_source, parameter_list, policy) -> ast.Module:
        '''Insert communication states into traget ast code.'''
        fragment_type = self.get_fragment_types(parameter_list)
        user_defined_buffer_name, learn_args, learn_return = self.find_buffer_and_learn_args(
            ast_source)
        ...
        for frag_type in fragment_type:
            ...
            top, bottom = self.gen_commuicate_statement(
                frag_type, communication_op, user_defined_buffer_name, framework_buffer_name,
                framework_weight_name, learn_args, learn_return)
            ast_target = self.insert_states(
                ast_target, frag_type, top, bottom)
        return ast_target
    ...
    def add_common_kernel(cls, ast_target):
        ...
        for key, var in kernel_names.items():
            ...
            kernel = str(key)
            if var:
                keyword = [ast.keyword(arg=list(var.keys())[0],
                                       value=ast.Name(id=list(var.values())[0],
                                       ctx=ast.Load()))]
            node = ast.Assign(targets=[ast.Attribute(value=ast.Name(id='self', ctx=ast.Load()),
                              attr=kernel.lower(), ctx=ast.Store())],
                              value=ast.Call(func=ast.Attribute(value=ast.Name(id='P', ctx=ast.Load()),
                              attr=kernel, ctx=ast.Load()), args=[], keywords=keyword))
    ...
    def create_fragment(self) -> list:
        ...
        ast_target = self.split_trainer_to_fragment(
            ast_target, ast_source)
