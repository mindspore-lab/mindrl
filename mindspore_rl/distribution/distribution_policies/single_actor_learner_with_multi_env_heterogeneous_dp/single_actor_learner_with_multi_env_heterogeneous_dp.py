...
class SingleActorLearnerMultiEnvHeterDP(DistributionPolicy):
    """define multi actor single learner policy"""

    def __init__(self, msrl, algorithm_config=None):
        # pylint: disable=R1725
        super(SingleActorLearnerMultiEnvHeterDP, self).__init__()
        ...
        self.set_boundary("actor-learner")
        self.add_interface("Actor", {"operations": {"Broadcast"}})
        self.add_interface("Learner", {"operations": {"Broadcast"}})
        self.add_communication_data(
            "Data", list(map(lambda b: b.name, msrl.buffers.buffer))
        )
        self.add_communication_data("Weight", None)
        self.set_replicate_list("Actor", self.actor_number)
        self.set_topology({"Learner": "Actor"})
        self.name = "SingleActorLearnerMultiEnvHeterDP"
        self.auto = True
