    ...
    class SACActorNet(nn.Cell):
        ...
        def __init__(
            ...
            for _, out_size in enumerate(hidden_sizes):
                model_list.append(
                    nn.Dense(in_size, out_size, weight_init="XavierUniform").to_float(
                        compute_type
                    )
                )
            ...
            self.last_fc = nn.Dense(
                in_size, output_size, weight_init="XavierUniform"
            ).to_float(compute_type)

            self.conditioned_std = conditioned_std
            if self.conditioned_std:
                self.last_fc_log_std = nn.Dense(
                    in_size, output_size, weight_init="XavierUniform"
                ).to_float(compute_type)
            else:
                self.action_log_std = Parameter(
                    Tensor(np.zeros((1, output_size)), mindspore.float32),
                    name="action_log_std",
                    requires_grad=True,
                )
            self.exp = ops.Exp()
...
class SACLearner(Learner):
    ...
    class CriticLossCell(nn.Cell):
        ...
        def construct(self, next_state, reward, state, action, done):
            ...
            next_action, next_log_prob = self.dist.sample_and_log_prob(
                (), next_means, next_stds
            )

            target_q_value1 = self.target_critic_net1(next_state, next_action).squeeze(
                axis=-1
            )
            target_q_value2 = self.target_critic_net2(next_state, next_action).squeeze(
                axis=-1
            )
            target_q_value = (
                self.min(target_q_value1, target_q_value2)
                - self.exp(self.log_alpha) * next_log_prob
            )
            td_target = (
                self.reward_scale_factor * reward
                + self.gamma * (1 - done.squeeze(axis=-1)) * target_q_value
            )
    ...
    class ActorLossCell(nn.Cell):
        ...
        def __init__(
            ...
            critic_net2,
            actor_mean_std_reg,
            actor_mean_reg_weight,
            actor_std_reg_weight,
        ):
    ...
    def __init__(self, params):
        ...
        actor_loss_net = SACLearner.ActorLossCell(
            ...
            params.get("actor_mean_reg_weight", 0.0),
            params.get("actor_std_reg_weight", 0.0),
        )
