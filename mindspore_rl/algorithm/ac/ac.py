...
class ACLearner(Learner):
    ...
    def __init__(self, params):
        ...
        optimizer_a = nn.Adam(
            self.actor_net.trainable_params(), learning_rate=params["alr"]
        )
        optimizer_c = nn.Adam(
            self.critic_net.trainable_params(), learning_rate=params["clr"]
        )
        ...
        critic_loss_net = self.CriticNNLoss(self.critic_net, self.gamma)
