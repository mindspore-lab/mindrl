...
if context.get_context("device_target") in ["Ascend"]:
    COMM_GROUP = HCCL_WORLD_COMM_GROUP
elif context.get_context("device_target") in ["GPU"]:
    COMM_GROUP = NCCL_WORLD_COMM_GROUP
else:
    raise RuntimeError("These distribute script is not supported in CPU.")
...
class A3CTrainer(Trainer):
    ...
    def __init__(self, msrl):
        ...
        self.send_actor = MuxSend(dest_rank=self.learner_rank, group=COMM_GROUP)
        self.recv_actor = MuxReceive(
            shape=shapes, dtype=mindspore.float32, group=COMM_GROUP
        )
        ...
        self.send_learner = MuxSend(dest_rank=-1, group=COMM_GROUP)
    ...
    def train_one_episode(self):
        ...
        if rank_id == self.learner_rank:  # learner
            ...
            result = self.msrl.actors[rank_id].act(
