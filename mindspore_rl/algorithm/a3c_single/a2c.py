...
from mindspore_rl.utils import DiscountedReturn, TensorArray
    ...
    class ActorCriticNet(nn.Cell):
        ...
        def __init__(
            ...
            self.critic = nn.Dense(
                hidden_size, 1, weight_init="XavierUniform"
            ).to_float(compute_type)
        ...
        def construct(self, x):
            x = self.common(x)
            x = self.relu(x)
            return self.cast(self.actor(x), mindspore.float32), self.cast(
                self.critic(x), mindspore.float32
            )
...
class A2CActor(Actor):
    ...
    def __init__(self, params=None):
        ...
        self.local_weight = ParameterTuple(self.a2c_net.trainable_params())
    ...
    def act(self, phase, params):
        ...
        if phase == 2:
            ...
            a2c_loss = self.loss_net(states, actions, normalized_returns)
            grads = self.grad(self.loss_net, self.local_weight)(
                *(states, actions, normalized_returns)
            )
            self.print("Loss ", a2c_loss)
            self.print("Reward ", done_num)
            return grads
