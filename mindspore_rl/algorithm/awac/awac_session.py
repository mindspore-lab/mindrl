...
class AWACLossCallback(Callback):
    """Two loss callback for AWAC"""
    ...
    def episode_end(self, params):
        ...
        if (params.cur_episode % self.interval) == 0:
            print("Episode {}: critic_loss is {}, actor_loss is {}, mean_std is {}, per_step_time {:5.3f} ms,".format(\
                  params.cur_episode, losses_out[0], losses_out[1], losses_out[2], epoch_ms/self.n_steps_per_episode),
                  flush=True)


class AWACSession(Session):
    '''AWAC session'''
    def __init__(self, env_yaml=None, algo_yaml=None):
        ...
        loss_cb = AWACLossCallback(config.trainer_params.get('loss_freq'),
