...
MonteCarloTreeNodePtr MonteCarloTreeFactory::CreateNode(
    const std::string &node_name, int *action, float *prior, float *init_reward,
  ...
  if (node_creator == map_node_name_to_node_creator_.end()) {
    ...
    oss << "Node register: [";
    for (auto iter = map_node_name_to_node_creator_.begin();
         iter != map_node_name_to_node_creator_.end(); iter++) {
      oss << iter->first << " ";
    }
    oss << "]";
  ...
  auto node = std::shared_ptr<MonteCarloTreeNode>(
      node_creator->second(node_name, action, prior, init_reward, player,
                           tree_handle, parent_node, row, state_size));
  return node;
}

std::tuple<int64_t, MonteCarloTreePtr> MonteCarloTreeFactory::CreateTree(
    const std::string &tree_name, const std::string &node_name, int player,
    float max_utility, int state_size, int total_num_player,
    float *input_global_variable) {
  handle_++;
  MonteCarloTreePtr tree;
  auto root = MonteCarloTreeFactory::GetInstance().CreateNode(
      node_name, nullptr, nullptr, nullptr, player, handle_, nullptr, 0,
      state_size);
  if (root == nullptr) {
  ...
  if (tree_creator == map_tree_name_to_tree_creator_.end()) {
    ...
    oss << "Tree register: [";
    for (auto iter = map_tree_name_to_tree_creator_.begin();
         iter != map_tree_name_to_tree_creator_.end(); iter++) {
      oss << iter->first << " ";
    }
    oss << "]";
    LOG_ERROR("%s", oss.str().c_str());
  ...
  tree = std::shared_ptr<MonteCarloTree>(tree_creator->second(
      root, max_utility, handle_, state_size, total_num_player));

  map_handle_to_tree_ptr_.insert(std::make_pair(handle_, tree));
  map_handle_to_tree_const_.insert(
      std::make_pair(handle_, input_global_variable));
  return std::make_tuple(handle_, tree);
}

void MonteCarloTreeFactory::InsertGlobalVariable(int64_t tree_handle,
                                                 float *global_variable) {
  map_handle_to_tree_variable_.insert(
      std::make_pair(tree_handle, global_variable));
}

void MonteCarloTreeFactory::RegisterNode(const std::string &node_name,
                                         NodeCreator &&node_creator) {
  map_node_name_to_node_creator_.insert(
      std::make_pair(node_name, node_creator));
}

void MonteCarloTreeFactory::RegisterTree(const std::string &tree_name,
                                         TreeCreator &&tree_creator) {
  map_tree_name_to_tree_creator_.insert(
      std::make_pair(tree_name, tree_creator));
}
...
MonteCarloTreePtr MonteCarloTreeFactory::GetTreeByHandle(int64_t handle) {
  ...
  if (iter == map_handle_to_tree_ptr_.end()) {
    ...
    oss << "Handle register: [";
    for (auto pairs = map_handle_to_tree_ptr_.begin();
         pairs != map_handle_to_tree_ptr_.end(); pairs++) {
      oss << pairs->first << " ";
    }
    oss << "]";
    LOG_ERROR("%s", oss.str().c_str());
...
float *MonteCarloTreeFactory::GetTreeVariableByHandle(int64_t handle) {
  ...
  if (iter == map_handle_to_tree_variable_.end()) {
    ...
    oss << "Handle register: [";
    for (auto pairs = map_handle_to_tree_variable_.begin();
         pairs != map_handle_to_tree_variable_.end(); pairs++) {
      oss << pairs->first << " ";
    }
    oss << "]";
    LOG_ERROR("%s", oss.str().c_str());
...
float *MonteCarloTreeFactory::GetTreeConstByHandle(int64_t handle) {
  ...
  if (iter == map_handle_to_tree_const_.end()) {
    ...
    oss << "The input handle " << handle
        << " in GetTreeVariableByHandle does not exist. ";
    oss << "Handle register: [";
    for (auto pairs = map_handle_to_tree_const_.begin();
         pairs != map_handle_to_tree_const_.end(); pairs++) {
      oss << pairs->first << " ";
    }
    oss << "]";
    LOG_ERROR("%s", oss.str().c_str());
